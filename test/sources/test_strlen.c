...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/12/14 12:23:06 by pguillie          #+#    #+#             */
/*   Updated: 2018/12/14 12:28:39 by pguillie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	unit_strlen(const char *s)
{
...
uint32_t	test_strlen(void)
{
	...
	ret |= (unit_strlen("") << 0);
	/* test 2 */
	ret |= (unit_strlen("hello world!") << 1);
	/* test 3 */
	ret |= (unit_strlen("Foo Bar Baz") << 2);
	/* test 4 */
	ret |= (unit_strlen("/A82lijPfGcM08l3vRh+5BQL/pvlEPER2hWNZF/8Oxe4p432p6VcgLts"
	...
	ret |= (unit_strlen("\n\nnewlines!\n\n") << 4);
	/* test 6 */
	ret |= (unit_strlen("\t!@#$%^&*()_+\r") << 5);
	/* test 7 */
	ret |= (unit_strlen("\t\n\v\f\r ") << 6);
	/* test 8 */
	ret |= (unit_strlen("qwertyuiopasdfghjklzxcvbnm") << 7);
	/* test 9 */
	ret |= (unit_strlen("fake\0null-string!") << 8);
